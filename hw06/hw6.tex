...
1. while the tape is not empty: \hspace{0.5cm} $\rightarrow O(|\langle x \rangle| x)$ \\~\\
\hspace*{0.9cm}
\begin{minipage}{.9\textwidth}
	a. count number of 1's on the tape and let $k$ be the number of 1's counted \\
	b. if $k \neq x$, reject \\
	c. else set $x$ to $x - 1$ and erase the last 1 on the tape\\ 
	d. repeat step 1 \\
\end{minipage}
	if $x = 0$, check that the tape is blank  
\end{minipage}
