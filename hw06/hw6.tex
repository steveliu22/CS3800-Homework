...
\hspace*{0.5cm}
\begin{minipage}{0.9\textwidth}
1. Mark the start 1* \hspace{0.5cm} $\rightarrow O(1)$ (The tape head should already be at the start.) \\~\\
2. While we are currently on a 1 or 1*: \hspace{0.5cm} $\rightarrow O(1)$
\begin{enumerate}[label=(\alph*),leftmargin=1.5cm]
	\item Mark where we are as 1* \hspace{0.5cm} $\rightarrow O(1)$
	\item Move right until we've found the last 1 \hspace{0.5cm} $\rightarrow O(|\langle x \rangle|)$ or $O(x)$ 
	\begin{enumerate}
		\item If we've found a 1 or $1$* (we will be stuck on $1$* if there are no more 1's left), replace it with a $\sqcup$ (empty cell) \hspace{0.5cm} $\rightarrow O(1)$
	\end{enumerate}
	\item Move left once \hspace{0.5cm} $\rightarrow O(1)$
	\item Move left until we're at a $1$* or $\sqcup$ \hspace{0.5cm} $\rightarrow O(|\langle x \rangle|)$ or $O(x)$ 
	\begin{enumerate}
		\item If we've found a $1$* replace it with 1 \hspace{0.5cm} $\rightarrow O(1)$ 
	\end{enumerate} 
\end{enumerate}
(The loop in step 2 will go for $O(|\langle x \rangle|)$ or $O(x)$ times.) \\~\\  
\end{minipage}
...
Let $M$ be the TM on this process, on input $\langle x \rangle$:
\begin{enumerate}
	\item 1. While there exists a 1 on the tape:
	\begin{enumerate}
	\item Move right until we've found the last 1 or 0 on the tape
	\item 
\end{enumerate}
\end{enumerate}
	  a. Move right until we've found the last 1 or 0 on the tape \\
	  If we are on a 1\\
